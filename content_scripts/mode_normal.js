class NormalMode extends KeyHandlerMode {
  ...
  commandHandler({ command: registryEntry, count }) {
    ...
    // BrowserToolbox 新命令仍走统一 Dispatcher，键盘输入与鼠标输入共享同一执行入口。
    if (registryEntry.command.startsWith("BrowserToolbox.")) {
      ...
      chrome.runtime.sendMessage({ handler: "browserToolbox.invoke", invocation });
      return;
    }
