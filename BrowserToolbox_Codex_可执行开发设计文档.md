# 浏览器工具箱可执行开发设计文档（Codex 实施版）

> **对外产品名**：浏览器工具箱（Browser Toolbox）
> **命名状态**：产品名已确定；图标归属、商标可用性和法律结论仍待检索确认
> **仓库与技术标识**：BrowserToolbox（当前统一用于代码、目录、命令、消息协议、存储键和测试；旧标识仅用于升级兼容迁移）
> **文档版本**：1.0  
> **编制日期**：2026-08-23  
> **主要目标平台**：Google Chrome、Microsoft Edge 及其他 Chromium 浏览器  
> **扩展规范**：Manifest V3  
> **代码基线**：Vimium `v2.4.2`，提交 `eb737ab`  
> **项目定位**：一个永久免费、开源、本地优先、无遥测的浏览器工作流工具箱；无广告、无账户、无订阅、无付费功能、无商业版
...
1. **不得从零重写 Vimium。** 第一版必须以 Vimium `v2.4.2` 为稳定基线，在不破坏原有键盘能力的前提下扩展浏览器工具箱模块。
...
浏览器工具箱是一个本地优先的浏览器工作流扩展，以统一命令系统连接输入、页面动作、浏览器控制和本地设置。
产品范围分为以下几类，鼠标手势只是其中一类输入模块：

- **键盘与命令**：Vim 风格键盘导航、链接提示、Vomnibar、查找、Visual、Marks、命令重复和键位配置；
- **页面与浏览器工作流**：链接、选中文本、图片、搜索、标签页、窗口、历史、书签和页面控制；
- **鼠标输入适配器**：鼠标轨迹、超级拖拽、滚轮和摇杆；
- **本地配置与安全边界**：设置、迁移、导入导出、站点规则、自定义指针、隐私和浏览器限制提示。

本节定义产品范围，不等于当前 checkout 的全部功能已经完成或通过人工验收；实际状态以
`docs/codex-progress.md`、`docs/feature-parity-matrix.md` 和当前测试输出为准。

### 1.2 Chrome Web Store 单一用途描述

“工具箱”在商店文案中仍必须保持一个聚焦用途，不代表把无关应用拼在一起。商店单一用途应固定为：

> 使用键盘和鼠标输入，统一执行网页导航、页面对象操作、搜索以及浏览器标签页和窗口工作流。

不得把 AI 聊天、广告拦截、密码管理、笔记、天气、新闻、购物、代理、下载管理器等不相关能力加入本项目。
竞品的完整功能清单不能直接转化为本项目需求；新增模块必须能说明其与浏览器工作流的关系、权限边界和本地运行方式。
...
4. 普通键盘输入和网页原生拖放优先受到保护；右键菜单按鼠标手势的菜单抑制开关处理，关闭开关时保留原生菜单。
...
- 把鼠标手势或任何单一竞品作为本产品的唯一定位；
- 为了追逐竞品功能而加入与浏览器工作流无关的应用集合；
...
CrxMouse 不是本产品的定义，也不是本项目的代码或资产来源。它只作为鼠标输入模块中部分公开行为的参考，
采用独立实现；不要求复刻 CrxMouse 的完整产品范围：
...
> **浏览器工具箱产品层 + Vimium 稳定基线 + 统一命令适配层 + 独立输入模块**
...
```bash
git clone https://github.com/philc/vimium.git browser-toolbox
cd browser-toolbox
...
在 `lib/browser_toolbox/command_invocation.js` 定义：
...
BrowserToolbox.toggleMouseGestures
BrowserToolbox.toggleSuperDrag
BrowserToolbox.showTabList
BrowserToolbox.copySelection
BrowserToolbox.copyLinkText
BrowserToolbox.copyImageUrl
BrowserToolbox.searchSelection
BrowserToolbox.openLinkForeground
BrowserToolbox.openLinkBackground
BrowserToolbox.openImageForeground
BrowserToolbox.openImageBackground
BrowserToolbox.downloadImage
BrowserToolbox.toggleFullscreen
BrowserToolbox.minimizeWindow
BrowserToolbox.maximizeWindow
```
...
- 可配置四向或八向轨迹手势；
- 右键按住绘制轨迹；
- 右键轻点或移动距离未达到激活阈值时显示原生菜单；启用右键轨迹的菜单抑制后，仅在越过激活阈值形成手势时阻止菜单；
- 手势轨迹线；
- 当前识别方向和目标命令 HUD；
- 未识别手势提示；
- 手势取消；
...
| C-001 | CrxMouse | Right-hold drag left = back | Store docs | goBack | PASS | PASS | DONE | 仅作为鼠标输入模块的 clean-room 行为参考 |
```
...
- 普通点击不得调用 `preventDefault()`；菜单抑制关闭时普通右键不得调用 `preventDefault()`；
- `contextmenu` 可能在首次 `pointermove` 前派发；因此 PENDING 阶段必须保留普通右键菜单，达到激活距离后再拦截后续 `contextmenu` 和轨迹事件；若浏览器已在激活前派发并放行菜单，扩展无法事后撤回该菜单；
...
1. 菜单抑制关闭时，右键按下后不移动或移动距离未达到阈值：原生菜单正常出现；
2. 菜单抑制开启时，右键输入在 PENDING 阶段仍保留原生 `contextmenu`；
3. 达到阈值并形成手势后：后续 `contextmenu` 不出现原生菜单；
4. ACTIVE 后按 Escape：取消手势且不执行命令；
5. 网页自行阻止右键菜单时，本扩展不尝试恢复网页已禁止的菜单；
6. 在设置中允许用户关闭菜单抑制或完全关闭右键手势。
...
以下仅定义浏览器工具箱鼠标输入模块的初始默认行为。设计时可以参考 CrxMouse 的公开可见行为，
但必须采用独立实现；CrxMouse 不构成本产品其他模块的需求来源：
...
| `U>R` | `nextTab` | 右侧标签页 |
| `D>R>U` | `BrowserToolbox.newWindow` | 新建窗口 |
| `D>R>D` | `BrowserToolbox.closeWindow` | 关闭当前窗口 |
...
- 验证后退、前进和关闭标签页；
- 验证菜单抑制开关关闭时的原生右键行为，以及开启时右键轨迹不被菜单抢占；
- 验证设置修改即时生效；
- 验证站点禁用；
- 验证超级拖拽打开前台/后台标签；
- 验证滚轮和摇杆；
...
### Phase 0：产品边界、仓库、许可证和基线

本阶段同时固定产品名、技术标识和竞品边界：对外建设的是浏览器工作流工具箱，
`BrowserToolbox` 是当前 checkout 的统一技术标识。旧技术标识只允许在配置、导出文件和本地资源的兼容读取路径出现；不得把兼容别名扩散到新的运行时调用或写入路径。
...
### Phase 1：浏览器工具箱统一命令 Invocation 和 Dispatcher
...
### Phase 7：工具箱边界、兼容性和无障碍

任务：

- [ ] 完成全部功能对照矩阵；
- [ ] 检查每个 Vimium 命令仍可用；
- [ ] 对鼠标输入模块涉及的 CrxMouse 公开行为逐项黑盒验证；不把 CrxMouse 的完整功能清单当作产品验收范围；
...
origin   = 用户自己的 BrowserToolbox 仓库
```
...
| 右键菜单冲突 | 用户无法正常浏览 | PENDING 阶段保留原生菜单，越过激活阈值后按配置接管；Chromium 若在首次移动前已派发并放行菜单，扩展无法事后撤回，关闭开关或改用非右键触发键可保留原生菜单 |
...
```text
你正在维护一个 Chrome Manifest V3 浏览器扩展项目，产品对外定位是“浏览器工具箱”：以 Vimium 的键盘导航为稳定基线，
通过统一命令层提供浏览器工作流，并把鼠标轨迹、超级拖拽、滚轮和摇杆作为独立输入模块接入。

请先完整阅读仓库中的《BrowserToolbox 可执行开发设计文档（Codex 实施版）》以及 AGENTS.md。
...
6. CrxMouse 只作为鼠标输入模块的公开行为参考；不解包、复制或改写其闭源代码和资产，只做 clean-room 独立实现。
...
> 一个以 Vimium 成熟键盘能力为基线、以统一命令系统为核心、由键盘、鼠标和浏览器工作流模块组成，
> 本地优先、零遥测、永久免费且可持续维护的开源浏览器工具箱。
