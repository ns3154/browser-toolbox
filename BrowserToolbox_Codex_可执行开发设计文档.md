...
- 可配置四向或八向轨迹手势；
- 右键按住绘制轨迹；
- 菜单抑制关闭时普通右键显示原生菜单；启用右键轨迹的菜单抑制时，从按下手势按键开始阻止 Chromium 菜单抢占轨迹；
- 手势轨迹线；
- 当前识别方向和目标命令 HUD；
- 未识别手势提示；
- 手势取消；
...
- Chromium 可能在首次 `pointermove` 前派发 `contextmenu`，因此菜单抑制开启时从手势会话开始保护；达到激活距离后继续拦截后续轨迹事件；
...
2. 菜单抑制开启时，从右键手势会话开始阻止 Chromium `contextmenu`，避免它在首次 `pointermove` 前抢占轨迹；
3. 达到阈值并形成手势：不出现原生菜单；
...
| 右键菜单冲突 | 用户无法正常浏览 | Chromium 可能在首次移动前派发 `contextmenu`；菜单抑制开启时从手势会话开始接管，关闭开关或改用非右键触发键可保留原生菜单 |
