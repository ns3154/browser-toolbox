...
以下记录当前开发中的变更；本仓库 `manifest.json` 版本为
`0.1.2`，不代表这些变更已通过商店审核或上线。

- 完善工具箱配置的同步容量校验，在保存前提示单项或总容量超限，并说明同步开关的适用范围。
- 为所有网站的会话暂停提供明确的恢复入口，保留用户已保存的功能设置和站点规则。
- 修复浏览器受限页面上的“全部工具”入口；从弹窗管理当前网站时，先打开规则草稿，保存后才生效。
- 统一构建入口和产物校验，增加完整检查命令及 CI，区分已通过、失败和跳过的检查。
- 更新使用指南、问题反馈模板和本地工具说明，补充时间工具的多语言无障碍文案。

## 浏览器工具箱 0.1.2（代码能力记录）

本节根据仓库现有实现整理，不作为商店发布状态记录。

- 提供 8 个本地工具：JSON 格式化、Properties ↔ YAML、文本对比、编码转换、时间转换、ID
  生成、密码生成、CSV / TSV 转换。
- 提供命令中心，可搜索命令、工具和设置；支持为网站选择配置档或独立调整行为。
- 鼠标手势与超级拖拽统一使用上下左右四方向；旧斜向绑定由设置迁移处理。
- 提供简体中文、繁体中文、英语、日语和西班牙语界面；右键菜单可选择全部支持该入口的工具。

## 浏览器工具箱 0.1.0（预发布，未完成最终兼容性验收）

- 基于 Vimium v2.4.2 键盘导航基线，保留原有键盘命令和 MIT 版权链。
- 增加独立实现的统一命令层、可配置鼠标轨迹、超级拖拽、滚轮、摇杆、站点规则、自定义本地指针和设置迁移。
- 增加中英文设置页、导入导出、无障碍键盘操作、Service Worker 重启恢复和本地安全/权限/网络审计。
- 项目永久免费、无广告、无账户、无订阅、无付费功能、无遥测、无远程代码，不复制 CrxMouse
  闭源代码、资产、 文案或界面。
- Windows、Edge、Chrome Stable 人工矩阵、认证态真实站点、屏幕阅读器和完整 Vimium
  手工回归仍待实际验证， 不应据此条目宣称最终发布完成。
