...
(function () {
  ...
  const FALLBACK_TOOL_SOURCES = new Map([
    ...
    ["config.convert", ["action", "selection", "command"]],
  ...
  const FALLBACK_TOOL_SURFACES = new Map([
    ...
    ["config.convert", ["catalog", "popup", "contextMenu", "settings"]],
  ...
  function validateToolIds(ids, name, max, source, surface, errors) {
    if (!Array.isArray(ids)) {
      errors.push(`${name} must be an array of tool IDs.`);
      return;
    }
    if (ids.length > max) {
  ...
  function validateTools(tools, errors) {
    ...
    validateToolIds(tools.pinnedIds, "tools.pinnedIds", Infinity, "action", "popup", errors);
  ...
  function validate(settings, registry) {
    ...
    if (!schema.SUPPORTED_LANGUAGES.includes(general.language)) {
