...
(function () {
  ...
  /**
   * 把当前 URL 命中的站点配置档覆盖到模块设置上。
   * 配置档只保存显式覆盖字段，未出现的字段继续继承全局设置。
   * @param {Object} settings 待应用覆盖的设置副本。
   * @param {string} url 顶层页面 URL。
   * @returns {Object|null} 当前生效的配置档。
   */
  function applyProfileOverrides(settings, url = "") {
    const profile = matcher.effectiveProfile(settings.siteRules, url);
    if (!profile) return null;
    for (const sectionName of ["mouse", "superDrag", "wheel", "rocker"]) {
      const overrides = profile[sectionName];
      if (!overrides || typeof overrides !== "object" || Array.isArray(overrides)) continue;
      settings[sectionName] = Object.assign({}, settings[sectionName] || {}, overrides);
    }
    return profile;
  }
  ...
  function resolveEffectiveModules(
    ...
    const source = schema.clone(settings || schema.DEFAULT_SETTINGS);
    applyProfileOverrides(source, url);
  ...
  function withEffectiveSettings(settings, url = "", options = {}) {
    ...
    value.effectiveSiteProfile = applyProfileOverrides(value, url);
