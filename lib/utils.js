...
const Utils = {
  ...
  addChromeRuntimeOnMessageListener(requestsHandled, listenerFn) {
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
      // 两套协议共用 runtime 通道；此监听器不处理 BrowserToolbox 消息，
      // 其版本、类型和发送方仍由 BrowserToolbox 自己的监听器校验。
      if (
        request?.handler == null && typeof request?.type === "string" &&
        request.type.startsWith("browserToolbox.")
      ) return false;
