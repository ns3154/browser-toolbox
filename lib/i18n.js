...
(function () {
  const fallbackMessages = {
    ...
    browserToolboxMouseHelp:
      "Draw a configured path with the gesture button. When menu suppression is enabled, Chromium context menus are blocked from button-down to prevent right-button gesture conflicts; disabling it keeps the native menu but may conflict with gestures.",
    ...
    mouseTriggerButtonHint:
      "The gesture button takes priority over super drag and rocker input when they use the same button. With the right button, menu suppression starts on button-down to prevent Chromium context-menu conflicts; disabling it keeps the native menu but may conflict with gestures.",
    ...
    suppressContextMenu: "Suppress context menu during gestures",
    privacyImmutable:
    ...
    onboardingGesture:
      "Mouse gestures keep the native context menu until the activation threshold is crossed.",
    onboardingDrag:
  ...
  const zhMessages = {
    ...
    browserToolboxMouseHelp:
      "使用手势按键画出已配置的轨迹。启用菜单抑制时会从按下手势按键开始拦截浏览器菜单，避免右键轨迹冲突；关闭后保留原生菜单，但右键轨迹可能与菜单冲突。",
    browserToolboxSuperDragHelp:
    ...
    mouseTriggerButtonHint:
      "手势按键与超级拖拽或摇杆使用同一按键时，手势优先。手势按键为右键时，菜单抑制会从按下开始，避免 Chromium 菜单抢占轨迹；关闭后保留原生菜单，但右键轨迹可能冲突。",
    directionMode: "方向模式",
    fourWay: "四向",
    eightWay: "八向",
    ...
    showTrail: "显示手势轨迹",
    showHud: "显示命令提示",
    suppressContextMenu: "手势期间抑制原生菜单",
    ...
    onboardingGesture:
      "启用菜单抑制时，右键轨迹从按下开始拦截冲突的原生菜单；关闭后保留原生菜单，但可能与轨迹冲突。",
