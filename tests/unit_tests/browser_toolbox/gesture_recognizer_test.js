...
context("Gesture recognizer", () => {
  ...
  should("distinguish exact matches and prefixes", () => {
    assert.equal("goBack", BrowserToolboxGestureRecognizer.find(["L"], bindings).exact.commandName);
    const prefix = BrowserToolboxGestureRecognizer.find(["D"], bindings);
    assert.isTrue(prefix.isPrefix);
    assert.equal(null, prefix.exact);
  });

  should("not execute an unknown pattern", () => {
    const result = BrowserToolboxGestureRecognizer.find(["U"], bindings);
  ...
  should("respect disabled and contextual bindings and format keys", () => {
    ...
    assert.equal(null, BrowserToolboxGestureRecognizer.find(["L"], contextual).exact);
    assert.equal(null, BrowserToolboxGestureRecognizer.find(["R"], contextual, "LINK").exact);
    assert.equal(
      "default-context",
      BrowserToolboxGestureRecognizer.find(["D"], contextual, "LINK")
        .exact.commandName,
    );
    assert.equal("L>R", BrowserToolboxGestureRecognizer.format(" L > R "));
    assert.equal("LINK:L>R", BrowserToolboxGestureRecognizer.key(["L", "R"], "LINK"));
  });
});
