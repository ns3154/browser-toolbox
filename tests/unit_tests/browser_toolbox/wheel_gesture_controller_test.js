...
context("Wheel gesture controller", () => {
  should("accumulate a threshold and respect cooldown", () => {
    const controller = new BrowserToolboxWheelGestureController();
  ...
  should("clear accumulation and cooldown when a physical button is released", () => {
    const controller = new BrowserToolboxWheelGestureController();
    const settings = { enabled: true, threshold: 80, cooldownMs: 180 };
    assert.equal(null, controller.handle({ buttons: 2, deltaY: 40, now: 1000, settings }));
    controller.release(2);
    assert.equal(null, controller.handle({ buttons: 2, deltaY: 40, now: 1001, settings }));
    assert.equal(
      { button: "RIGHT_BUTTON", direction: "DOWN" },
      controller.handle({ buttons: 2, deltaY: 40, now: 1002, settings }),
    );
    controller.release(2);
    assert.equal(null, controller.handle({ buttons: 2, deltaY: 40, now: 1003, settings }));
    controller.handle({ buttons: 2, deltaY: 40, now: 1004, settings });
    assert.equal(null, controller.handle({ buttons: 0, deltaY: 0, now: 1005, settings }));
    assert.equal(null, controller.handle({ buttons: 2, deltaY: 40, now: 1006, settings }));
    assert.equal("RIGHT_BUTTON", BrowserToolboxWheelButtonName(2));
  });
});
